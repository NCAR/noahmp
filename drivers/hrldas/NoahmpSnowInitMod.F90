    ...
    do J = NoahmpIO%jts, jtf
       do I = NoahmpIO%its, itf
          do N=1, NoahmpIO%NumberOfTiles(I,J) 
             ...
             if ( NoahmpIO%SNOWH(I,J,N) < 0.025 ) then
                NoahmpIO%ISNOWXY(I,J,N) = 0
                DZSNO(-NoahmpIO%NSNOW+1:0) = 0.0
             else
                ! 1 layer snow
                if ( (NoahmpIO%SNOWH(I,J,N) >= 0.025) .and. (NoahmpIO%SNOWH(I,J,N) <= 0.05) ) then
                   NoahmpIO%ISNOWXY(I,J,N) = -1
                   DZSNO(0)  = NoahmpIO%SNOWH(I,J,N)
                ! 2 layer snow
                elseif ( (NoahmpIO%SNOWH(I,J,N) > 0.05) .and. (NoahmpIO%SNOWH(I,J,N) <= 0.10) ) then
                   NoahmpIO%ISNOWXY(I,J,N) = -2
                   DZSNO(-1) = NoahmpIO%SNOWH(I,J,N) / 2.0
                   DZSNO( 0) = NoahmpIO%SNOWH(I,J,N) / 2.0
                ! 2 layer thick snow
                elseif ( (NoahmpIO%SNOWH(I,J,N) > 0.10) .and. (NoahmpIO%SNOWH(I,J,N) <= 0.25) ) then
                   NoahmpIO%ISNOWXY(I,J,N) = -2
                   DZSNO(-1) = 0.05
                   DZSNO( 0) = NoahmpIO%SNOWH(I,J,N) - DZSNO(-1)
                ! 3 layer snow
               elseif ( (NoahmpIO%SNOWH(I,J,N) > 0.25) .and. (NoahmpIO%SNOWH(I,J,N) <= 0.45) ) then
                   NoahmpIO%ISNOWXY(I,J,N) = -3
                   DZSNO(-2) = 0.05
                   DZSNO(-1) = 0.5 * (NoahmpIO%SNOWH(I,J,N)-DZSNO(-2))
                   DZSNO( 0) = 0.5 * (NoahmpIO%SNOWH(I,J,N)-DZSNO(-2))
                ! 3 layer thick snow
               elseif ( NoahmpIO%SNOWH(I,J,N) > 0.45 ) then
                   NoahmpIO%ISNOWXY(I,J,N) = -3
                   DZSNO(-2) = 0.05
                   DZSNO(-1) = 0.20
                   DZSNO( 0) = NoahmpIO%SNOWH(I,J,N) - DZSNO(-1) - DZSNO(-2)
               else
             ...
             NoahmpIO%TSNOXY (I,-NoahmpIO%NSNOW+1:0,J,N) = 0.0
             NoahmpIO%SNICEXY(I,-NoahmpIO%NSNOW+1:0,J,N) = 0.0
             NoahmpIO%SNLIQXY(I,-NoahmpIO%NSNOW+1:0,J,N) = 0.0
             do IZ = NoahmpIO%ISNOWXY(I,J,N)+1, 0
                NoahmpIO%TSNOXY(I,IZ,J,N)  = NoahmpIO%TGXY(I,J,N)
                NoahmpIO%SNLIQXY(I,IZ,J,N) = 0.0
                NoahmpIO%SNICEXY(I,IZ,J,N) = 1.0 * DZSNO(IZ) * (NoahmpIO%SNOW(I,J,N)/NoahmpIO%SNOWH(I,J,N))
             enddo
             ...
             if ( NoahmpIO%IOPT_ALB == 3 )then
                NoahmpIO%SNRDSXY(I,-NoahmpIO%NSNOW+1:0,J,N)  = 0.0
                NoahmpIO%SNFRXY (I,-NoahmpIO%NSNOW+1:0,J,N)  = 0.0
                NoahmpIO%BCPHIXY(I,-NoahmpIO%NSNOW+1:0,J,N)  = 0.0
                NoahmpIO%BCPHOXY(I,-NoahmpIO%NSNOW+1:0,J,N)  = 0.0
                NoahmpIO%OCPHIXY(I,-NoahmpIO%NSNOW+1:0,J,N)  = 0.0
                NoahmpIO%OCPHOXY(I,-NoahmpIO%NSNOW+1:0,J,N)  = 0.0
                NoahmpIO%DUST1XY(I,-NoahmpIO%NSNOW+1:0,J,N)  = 0.0
                NoahmpIO%DUST2XY(I,-NoahmpIO%NSNOW+1:0,J,N)  = 0.0
                NoahmpIO%DUST3XY(I,-NoahmpIO%NSNOW+1:0,J,N)  = 0.0
                NoahmpIO%DUST4XY(I,-NoahmpIO%NSNOW+1:0,J,N)  = 0.0
                NoahmpIO%DUST5XY(I,-NoahmpIO%NSNOW+1:0,J,N)  = 0.0
             endif
          enddo ! N
       enddo    ! I
    enddo       ! J
