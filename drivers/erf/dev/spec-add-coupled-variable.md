...
The **order** of members there *is* the ABI order — the ABI being the binary
contract the C++ and Fortran sides must agree on (see the glossary in
[`spec-overview.md`](spec-overview.md) §7). One annotated declaration expands into
every place that contract needs it. Only the tracked `*-mc` templates are
hand-edited; the targets are gitignored and regenerated by the build. See
[`spec-fc-api.md`](spec-fc-api.md) for the full mechanism.
