...
contains
  ...
  subroutine SnowAgeSnicar(noahmp)
...
! local variable
    ...
    real(kind=kind_noahmp)           :: C2_liq_Brun89     = 4.22e-13         ! Constant for liquid water grain growth [m3 s-1], from Brun89: corrected for LWC in units of percent
    real(kind=kind_noahmp)           :: SnowRadiusRefrz   = 1000.0           ! Effective radius of re-frozen snow (microns)
...
! ----------------------------------------------------------------------
    ...
    do LoopInd=SnowLayerTop,SnowLayerBottom,1
      ...
      dr_wet = 1E18*(MainTimeStep*(C2_liq_Brun89*(FracLiqWater**(3))) / &
                     (4*ConstPI*SnowRadius(LoopInd)**(2)))

      dr = dr + dr_wet

      !**********  3. INCREMENT EFFECTIVE RADIUS, ACCOUNTING FOR:  ***********
      ! DRY AGING
      ! WET AGING
      ! FRESH SNOW
      ! RE-FREEZING
      ...
      !if (LoopInd == SnowLayerTop) then
      !   snot_top(c_idx)    = t_soisno(LoopInd)
      !   dTdz_top(c_idx)    = TemperatureGradient(LoopInd)
      !   snw_rds_top(c_idx) = SnowRadius(LoopInd)
      !   sno_liq_top(c_idx) = h2osoi_liq(LoopInd) / (h2osoi_liq(LoopInd)+h2osoi_ice(LoopInd))
      !endif
  ...
  end subroutine SnowAgeSnicar

end module SnowAgeSnicarMod
